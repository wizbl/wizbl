...
void PeerLogicValidation::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    LogPrintfd("pindexNew[%X]->nHeight[%d], pindexFork[%X], fInitialDownload[%d]", pindexNew, pindexNew->nHeight, pindexFork, fInitialDownload);
...
void static ProcessGetData(CNode* pfrom, const Consensus::Params& consensusParams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            LogPrintfd("inv.type[%d]", inv.type);
            LogPrintfd("inv.hash[%s]", inv.hash.ToString());
...
{
    LogPrintfd("strCommand[%s]", strCommand);
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION)
        {
            ...
            // TODO(h4x3rotab): Disconnect if we already have any WBL block.
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        for (CInv &inv : vInv)
        {
            LogPrintfd("interruptMsgProc[%d]", interruptMsgProc);
            if (interruptMsgProc)
                return true;
            ...
            else
            {
                ...
                LogPrintfd("fBlocksOnly[%d]", fBlocksOnly);
                LogPrintfd("else if (!fAlreadyHave[%d] && !fImporting[%d] && !fReindex[%d] && !IsInitialBlockDownload()[%d])[%d]",
                    fAlreadyHave, fImporting, fReindex, IsInitialBlockDownload(), (!fAlreadyHave && !fImporting && !fReindex && !IsInitialBlockDownload()));
                if (fBlocksOnly) {
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/wizblcoin/wizblcoin/issues/8279 for details.
                            assert(recentRejects);
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    LogPrintfd("pfrom->GetRecvVersion()[%d]", pfrom->GetRecvVersion());
...
bool SendMessages(CNode* pto, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!fReindex && !fImporting && !IsInitialBlockDownload()) {
            //LogPrintfd("GetMainSignals().Broadcast(nTimeBestReceived, &connman);");
        ...
        //
        {
            ...
            LogPrintfd("if (!fRevertToInv)[%d]", (!fRevertToInv));
            if (!fRevertToInv) {
                //LogPrintfd("");
                ...
                LogPrintfd("pto->vBlockHashesToAnnounce.size()[%d]", pto->vBlockHashesToAnnounce.size());
                for (const uint256 &hash : pto->vBlockHashesToAnnounce) {
                    BlockMap::iterator mi = mapBlockIndex.find(hash);
                    assert(mi != mapBlockIndex.end());
                    const CBlockIndex *pindex = mi->second;
                    if (chainActive[pindex->nHeight] != pindex) {
                        LogPrintfd("if (chainActive[pindex->nHeight] != pindex)");
                        // Bail out if we reorged away from this block
                        fRevertToInv = true;
                        break;
                    }
                    if (pBestIndex != nullptr && pindex->pprev != pBestIndex) {
                        LogPrintfd("if (pBestIndex != nullptr && pindex->pprev != pBestIndex)");
                    ...
                    if (fFoundStartingHeader) {
                        LogPrintfd("if (fFoundStartingHeader)");
                        // add this to the headers message
                        vHeaders.push_back(pindex->GetBlockHeader());
                    } else if (PeerHasHeader(&state, pindex)) {
                        LogPrintfd("else if (PeerHasHeader(&state, pindex[%X]))", pindex);
                        continue; // keep looking for the first new block
                    } else if (pindex->pprev == nullptr || PeerHasHeader(&state, pindex->pprev)) {
                        LogPrintfd("else if (pindex->pprev[%X] == nullptr || PeerHasHeader(&state, pindex->pprev))", pindex->pprev);
            ...
            LogPrintfd("if (!fRevertToInv[%d] && !vHeaders.empty()[%d])[%d]", fRevertToInv, vHeaders.empty(), (!fRevertToInv && !vHeaders.empty()));
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (fSendTrickle) {
                ...
                if (!pto->fRelayTxes)
                {
                    LogPrintfd("pto->setInventoryTxToSend.clear();");
            ...
            if (fSendTrickle && pto->fSendMempool) {
                ...
                for (const auto& txinfo : vtxinfo) {
                    ...
                    LogPrintfd("pto->setInventoryTxToSend.erase(hash);");
                    pto->setInventoryTxToSend.erase(hash);
                    //if (filterrate) {
                    //    LogPrintfd("if (txinfo.feeRate.GetFeePerK()[%s] < filterrate[%s])", FormatMoney(txinfo.feeRate.GetFeePerK()), FormatMoney(filterrate));
            ...
            if (fSendTrickle) {
                ...
                for (std::set<uint256>::iterator it = pto->setInventoryTxToSend.begin(); it != pto->setInventoryTxToSend.end(); it++) {
                    LogPrintfd("vInvTx.push_back(it[%s]);", it->ToString());
                    vInvTx.push_back(it);
                }
                //CAmount filterrate = 0;
                //{
                ...
                while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                    ...
                    LogPrintfd("pto->setInventoryTxToSend.erase(it[%s]);", it->ToString());
                    pto->setInventoryTxToSend.erase(it);
                    // Check if not in the filter already
                    if (pto->filterInventoryKnown.contains(hash)) {
                        LogPrintfd("if (pto->filterInventoryKnown.contains(hash)) {");
                        continue;
                    }
                    ...
                    //LogPrintfd("if (filterrate[%s] && txinfo.feeRate.GetFeePerK()[%s] < filterrate)", FormatMoney(filterrate), FormatMoney(txinfo.feeRate.GetFeePerK()));
                    ...
                    if (vInv.size() == MAX_INV_SZ) {
                        LogPrintfd("connman.PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv[%d]));", vInv.size());
                        connman.PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
                        vInv.clear();
                    }
                    LogPrintfd("pto->filterInventoryKnown.insert(hash[%s]);", hash.ToString());
                    pto->filterInventoryKnown.insert(hash);
                }
            }
        }
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            //LogPrintfd("currentFilter[%s]", FormatMoney(currentFilter));
            ...
            if (timeNow > pto->nextSendTimeFeeFilter) {
                ...
                LogPrintfd("filterToSend[%s]", FormatMoney(filterToSend));
