    ...
    nNewFee(0)
{
    ...
    if (totalFee > 0) {
        ...
        //LogPrintfd("");
        //CAmount minTotalFee = CFeeRate().GetMinFee(&wtx.tx->vout);
        ...
        //    vErrors.push_back(strprintf("Insufficient totalFee, must be at least %s", FormatMoney(minTotalFee)));
        //    currentResult = BumpFeeResult::INVALID_PARAMETER;
        //    return;
        //}
        //CAmount requiredFee = CWallet::GetRequiredFee(maxNewTxSize);
        //LogPrintfd("");
        //CAmount requiredFee = CFeeRate().GetMinFee(&wtx.tx->vout);
        ...
        LogPrintfd("");
        nNewFee = 1;//CFeeRate().GetMinFee(&wtx.tx->vout);
        nNewFeeRate = CFeeRate();
        ...
        //if (nNewFeeRate.GetFeePerK() < nOldFeeRate.GetFeePerK() + 1 + walletIncrementalRelayFee.GetFeePerK()) {
        //    nNewFeeRate = CFeeRate(nOldFeeRate.GetFeePerK() + 1 + walletIncrementalRelayFee.GetFeePerK());
        //    nNewFee = CFeeRate().GetMaxFee(wtx.tx);
        //}
    }

    // Check that in all cases the new fee doesn't violate maxTxFee
     if (0 != maxTxFee && nNewFee > maxTxFee) {
    ...
    //    vErrors.push_back(strprintf("New fee rate (%s) is less than the minimum fee rate (%s) to get into the mempool."
    //        " totalFee value should to be at least %s or settxfee value should be at least %s to add transaction.",
    //        FormatMoney(nNewFeeRate.GetFeePerK()), FormatMoney(minMempoolFeeRate.GetFeePerK()),
    //        FormatMoney(minMempoolFeeRate.GetFee(maxNewTxSize)), FormatMoney(minMempoolFeeRate.GetFeePerK())));
    //    currentResult = BumpFeeResult::WALLET_ERROR;
    //    return;
    //}
