...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addnode", tfm::format("\"192.168.0.6:%d\" \"onetry\"", DEFAULT_SERVER_PORT))
            + HelpExampleRpc("addnode", tfm::format("\"192.168.0.6:%d\", \"onetry\"", DEFAULT_SERVER_PORT))
        );
...
UniValue disconnectnode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("disconnectnode", tfm::format("\"192.168.0.6:%d\"", DEFAULT_SERVER_PORT))
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", tfm::format("\"192.168.0.6:%d\"", DEFAULT_SERVER_PORT))
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:%d\",  (string) The " TEXT_wizbl "coin server IP and port we're connected to\n"
