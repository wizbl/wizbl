...
{
	LogPrintfd("addrConnect => %s", addrConnect.ToString());
    if (pszDest == nullptr) {
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Wizblcoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = TEXT_Wizbl "coin " + FormatFullVersion();
...
static std::string GetDNSHost(const CDNSSeedData& data, ServiceFlags* requiredServiceBits)
{
	...
	LogPrintfd("data.supportsServiceBitsFiltering => %d", data.supportsServiceBitsFiltering);
	LogPrintfd("requiredServiceBits => %d", *requiredServiceBits);
...
void CConnman::ThreadDNSAddressSeed()
{
	...
	for (const CDNSSeedData &seed : vSeeds) {
		LogPrintfd("interruptNet => %d", (bool)interruptNet);
        if (interruptNet) {
            return;
		}
		LogPrintfd("HaveNameProxy() => %d", HaveNameProxy());
		LogPrintfd("seed.host => %s", seed.host);
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, 0, true))
            {
                for (const CNetAddr& ip : vIPs)
                {
					...
					LogPrintfd("addr => %s", addr.ToString());
                    vAdd.push_back(addr);
                    found++;
                }
                addrman.Add(vAdd, resolveSource);
				LogPrintfd("addrman.size() => %d", addrman.size());
            }
        }
    }
...
void CNode::ResetFilterInventoryKnown()
{
    LOCK(cs_inventory);
    filterInventoryKnown.reset();
}
void CNode::PushInventory(const CInv& inv)
{
    ...
    LOCK(cs_inventory);
    if (inv.type == MSG_TX) {
        LogPrintfd("if (!filterInventoryKnown.contains(inv.hash[%s])) {", inv.hash.ToString());
        if (!filterInventoryKnown.contains(inv.hash)) {
            LogPrintfd("setInventoryTxToSend.insert(inv.hash[%s]);", inv.hash.ToString());
