...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry& entry, bool validFeeEstimate)
{
    ...
    LogPrintfd("if (txHeight[%d] != nBestSeenHeight[%d])[%d]", txHeight, nBestSeenHeight, (txHeight != nBestSeenHeight));
    ...
    LogPrintfd("if (!validFeeEstimate)[%d]", validFeeEstimate);
    if (!validFeeEstimate) {
        untrackedTxs++;
        return;
    }
    ...
    // Feerates are stored and reported as WBL-per-kb:
...
{
    LogPrintfd("if (!removeTx(entry->GetTx().GetHash()[%s], true))[%d]", entry->GetTx().GetHash().ToString(), (!removeTx(entry->GetTx().GetHash(), true)));
    ...
    LogPrintfd("(nBlockHeight[%d] - entry->GetHeight()[%d])[%d]", nBlockHeight, entry->GetHeight(), nBlockHeight - entry->GetHeight());
