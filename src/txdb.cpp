    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->iGenerateID          = diskindex.iGenerateID;
                pindexNew->iVerificationLength  = diskindex.iVerificationLength;
                pindexNew->sig                  = diskindex.sig;

                // TODO(h4x3rotab): Check Equihash solution? Not sure why Zcash doesn't do it here.
                //bool postfork = 0/*pindexNew->nHeight >= consensusParams.WBLHeight*/;
                //if (!CheckProofOfWork(pindexNew->GetBlockHash(), pindexNew->nBits, postfork, consensusParams))
                //    return error("%s: CheckProofOfWork failed: %s", __func__, pindexNew->ToString());
                CBlockHeader tempBlock = pindexNew->GetBlockHeader();
                if (!Params().GetConsensus().SignatureValidation.verifyBlockHeader(&tempBlock))
                    return error("%s: verifyBlockHeader failed: %s", __func__, pindexNew->ToString());
