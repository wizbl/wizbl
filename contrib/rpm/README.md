...
The RPM spec file provided here is for Wizblcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://wizblcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Wizblcoin git repository](https://github.com/wizblcoin/wizblcoin) but are not part of
...
Currently only `Source10` is used, for the example `wizblcoincore.conf` file.
...
in the wizblcoin git repository. At present only an SVG version of the Wizblcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the wizblcoin
...
needed when 0.12.1 is released, a proper fix is already in the Wizblcoin git
master branch.
...
idea to build Wizblcoin Core against a static version of that library compiled
...
The `wizblcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `wizblcoin-server` package. At this point, most RPM based
...
The `wizblcoin-server` package should have SELinux support. How to properly do
...
Wizblcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/wizblcoin/). He has been
packaging Wizblcoin for Fedora at least since 2012.
...
4. I renamed the `wizblcoin` package that contains the Qt GUI to `wizblcoin-core` as
that appears to be how the general population refers to it, in contrast to
`wizblcoin-xt` or `wizblcoin-classic`. I wanted to make sure the general population
