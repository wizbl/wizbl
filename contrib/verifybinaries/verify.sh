...
###   wizblcoincore.org and wizblcoin.org and compares them.
...
WORKINGDIR="/tmp/wizblcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://wizblcoincore.org"
HOST2="https://wizblcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="wizblcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'wizblcoin-',
...
if [ $? -ne 0 ]; then
   echo "wizblcoin.org failed to provide signature file, but wizblcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "wizblcoin.org and wizblcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Wizblcoin Core binary release signing key installed?"
   fi
