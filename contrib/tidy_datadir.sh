#!/bin/bash
# Copyright (c) 2013 The Wizblcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Wizblcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Wizblcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Wizblcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Wizblcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Wizblcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Wizblcoin 0.8 datadir."
    ;;
esac
