...
*Setup instructions for a Gitian build of Wizblcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Wizblcoin
...
- [Building Wizblcoin Core](#building-wizblcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-wizblcoin-host/README.md).
...
Clone the git repositories for WIZBLCOIN and Gitian.
...
in the wizblcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Wizblcoin Core
----------------

To build Wizblcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the wizblcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/WIZBLCOIN/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/wizbl/wizbl
...
For example:
```bash
URL=https://github.com/akx20000a/WIZBLCOIN.git
COMMIT=d5522ed66a924a2d505fe45ed2487e741c6c0917
./bin/gbuild --commit WIZBLCOIN=${COMMIT} --url WIZBLCOIN=${URL} ../WIZBLCOIN/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit WIZBLCOIN=${COMMIT} --url WIZBLCOIN=${URL} ../WIZBLCOIN/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit WIZBLCOIN=${COMMIT} --url WIZBLCOIN=${URL} ../WIZBLCOIN/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the wizblcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../wizblcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/wizbl/wizblcoincore-detached-sigs.git

BTCPATH=/some/root/path/WIZBLCOIN
SIGPATH=/some/root/path/wizblcoincore-detached-sigs

./bin/gbuild --url WIZBLCOIN=${BTCPATH},signature=${SIGPATH} ../WIZBLCOIN/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wizblcoin-core-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wizblcoin-core-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/wizblcoin-core-osx-build.assert
```
